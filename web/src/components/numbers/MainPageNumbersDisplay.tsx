...
export default function MainPageNumbersDisplay({
    className = ''
}: Props) {
    ...
    // Function to get balanced distribution of numbers
    const getBalancedNumbers = (numbers: AvailableNumber[]) => {
        const categories = {
            simple: numbers.filter(r => {
                const catKey = (r.category ?? getCategory(r.monthlyFee).key) as string;
                return catKey === 'simple';
            }),
            gold: numbers.filter(r => {
                const catKey = (r.category ?? getCategory(r.monthlyFee).key) as string;
                return catKey === 'gold';
            }),
            platinum: numbers.filter(r => {
                const catKey = (r.category ?? getCategory(r.monthlyFee).key) as string;
                return catKey === 'platinum';
            }),
            vip: numbers.filter(r => {
                const catKey = (r.category ?? getCategory(r.monthlyFee).key) as string;
                return catKey === 'vip';
            })
        };
        ...
        const balanced = [
            ...categories.simple.slice(0, 2),
            ...categories.gold.slice(0, 2),
            ...categories.platinum.slice(0, 2),
            ...categories.vip.slice(0, 2)
        ];

        // If we don't have enough from each category, fill with others
        const remaining = numbers.filter(r => !balanced.includes(r));
        return [...balanced, ...remaining].slice(0, 6);
    };
    ...
    const displayedData = useMemo(() => {
        ...
        return getBalancedNumbers(filteredData);
    }, [filteredData, showAll]);

    const mobileRows = displayedData.filter(r => r.mobileNumber);
    const tfRows = displayedData.filter(r => r.number800);
        ...
        <div className={`rounded-2xl border-2 border-[#0A7B75]/20 bg-gradient-to-br from-white to-[#0A7B75]/5 shadow-xl ${className}`}>
            ...
            <div className="p-6 border-b border-[#0A7B75]/20 bg-gradient-to-r from-[#0A7B75]/5 to-transparent">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-gradient-to-r from-[#0A7B75] to-[#0A7B75]/90 text-white rounded-xl text-sm font-medium hover:from-[#0A7B75]/90 hover:to-[#0A7B75] transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                    >
                        {showAll ? 'Показать меньше' : `Показать все (${filteredData.length})`}
                    </button>
                </div>
            </div>
                        ...
                        {(filterView === 'all' || filterView === 'mobile') && mobileRows.length > 0 && (
                            <>
                                <tr className="bg-gradient-to-r from-[#0A7B75]/10 to-[#0A7B75]/5">
                                    <td className="px-6 py-4 text-sm font-bold text-[#0A7B75] border-l-4 border-[#0A7B75]" colSpan={6}>
            ...
            <div className="p-6 border-t border-[#0A7B75]/20 bg-gradient-to-r from-[#0A7B75]/5 to-transparent flex justify-between items-center">
                <div className="text-sm text-[#0A7B75] font-medium">
                    Показано: {displayedData.length} из {filteredData.length} номеров
                </div>
                <button
                    onClick={reload}
