...
# Usage: .\scripts\set-env.ps1 [local|production|render]
...
Write-Host "🔄 Setting environment to $Environment..." -ForegroundColor Green

# Set environment variable for current session
$env:NODE_ENV = $Environment

# Display current environment
Write-Host "✅ Environment set to: $env:NODE_ENV" -ForegroundColor Green
...
Write-Host "💡 Environment variable is set for current PowerShell session only" -ForegroundColor Yellow
